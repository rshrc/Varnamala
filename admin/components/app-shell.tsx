...
import { BookOpen, FileCheck2, LayoutDashboard, ListTodo, LoaderCircle, Menu, Settings, ShieldCheck, Users, X } from "lucide-react";
...
function NavigationPendingIndicator() {
  ...
  return <span className="ml-auto grid size-4 place-items-center" aria-hidden>{pending && <LoaderCircle className="size-3.5 animate-spin" />}</span>;
}
...
export function AppShell({ children, demo, name, role }: { children: React.ReactNode; demo: boolean; name: string; role: "moderator" | "admin" }) {
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[236px_1fr]">
    <button onClick={() => setOpen(true)} className="focus-ring fixed top-4 left-4 z-30 rounded-lg border border-line bg-surface p-2 shadow-sm lg:hidden" aria-label="Open menu"><Menu className="size-5" /></button>
    {open && <button className="fixed inset-0 z-30 bg-slate-950/25 lg:hidden" onClick={() => setOpen(false)} aria-label="Close menu overlay" />}
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[236px] flex-col border-r border-line bg-surface p-4 transition-transform lg:sticky lg:top-0 lg:h-screen ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
      ...
      <nav className="mt-7 space-y-1" aria-label="Main navigation">{nav.filter(([, , , minimumRole]) => minimumRole === "moderator" || role === "admin").map(([href, label, Icon]) => { const active = pathname === href || (href !== "/dashboard" && pathname.startsWith(href)); return <Link key={href} href={href} aria-current={active ? "page" : undefined} onClick={() => setOpen(false)} className={`focus-ring soft-transition flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium ${active ? "bg-brand-soft text-brand-dark" : "text-slate-600 hover:bg-slate-100 hover:text-ink"}`}><Icon className="size-[18px]" strokeWidth={1.8} />{label}<NavigationPendingIndicator /></Link>; })}</nav>
